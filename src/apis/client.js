...
class ApiClient {
  ...
  _getClient(baseURL, cookieToken) {
    const apiClient = axios.create({
      baseURL: baseURL || process.env.NEXT_PUBLIC_BACKEND_URL || "https://localhost:3000",
    });
  ...
  _get(url, config) {
    const get = this._getClient(undefined, config?.cookieToken).get(url, config);
    return get;
  }
  ...
  _cancelTokenRequest() {
    ...
    return async (method, url, data, headers) => {
      ...
      if (!authToken.state.token) return [undefined, "No token found", undefined];
